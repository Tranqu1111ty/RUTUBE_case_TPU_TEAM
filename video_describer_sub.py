...
def ret_stt(stt_name):
    with open(f"./submit/test/test_stt/{stt_name}", 'r', encoding="utf_8_sig") as f:
        lines = f.readlines()
...
path2csv = "submit/test/test.csv"
path2videos = "submit/test/test_video/"

data = process_corpus(path2csv)

video_names = data["video_name"].values
corpuses = data["stt_sum"].values
descs = [calc(corpuses[i], video_names[i], path2videos) for i in range(data.shape[0])]

data["generated_description"] = descs

data = data.drop(columns=['stt_name', 'category_name', 'stt', 'stt_sum'])
